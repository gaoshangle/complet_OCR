...
def get_min_distance_index(tem,indexes,near):
    ...
    for i in indexes:
        ...
        if distance_tem < distance_min and len(near[i]['text']):
            ii = i
...
def init_args():
    ...
    parser.add_argument('--model_path', default=r'/home/share/gaoluoluo/模型/model_best.pth', type=str)
...
def test(args, file=None):
    ...
    for img_path in tqdm(get_file_list(args.input_folder, p_postfix=['.jpg','.png'])):  # img_path /home/share/gaoluoluo/dbnet/test/input/2018实验仪器发票.jpg
        ...
        for i in range(0,len(points_kuang)):
...
def isTitle(text):
    ...
    return re.search(r'(货物|劳务|服.?名称|规.?型号|^单[位价]|^数量|.?额$|税率|税额)', text) != None
...
def get_basic_boundary(data):
    indexes = [i for i, d in enumerate(data) if re.search(r'开.?日期|票日期|校.?码|2?年?月', d['text'])]
    # print(data[71])
    if len(indexes):
        end = max(indexes)
    else:
...
def get_basic_date(basic):
    ...
    candidates = [d for d in basic if re.search(r'2?年?月', d['text'])]
...
def get_basic_person(data, boundary):
    ...
    indexes = [i for i,d in enumerate(rear) if re.search(r'收?款|开票人|复?核|人:|复.:', d['text'])]
    finded = len(indexes)
    if finded < 3:
        s = min(indexes) - 3
        ...
        if len(payee_text)>4:
            ...
            for i in range(len(data)-10,len(data)):
                ...
                if distance_tmp < distance_min and len(tem_payee['text']):
                    index = i
...
#--------------------------------------------
        ...
        if len(reviewer_text) > 3:
            ...
            for i in range(len(data)-10,len(data)):
                ...
                distance_tmp = math.sqrt(math.pow((x - x_tem),2) + math.pow((y - y_tem),2))
                if distance_tmp < distance_min and len(tem_reviewer['text']):
                    distance_min = distance_tmp
                    index  = i
...
#--------------------------------------------------------
        ...
        if len(drawer_text) > 4:
            ...
            for i in range(len(data)-10,len(data)):
                ...
                distance_tmp = math.sqrt(math.pow((x - x_tem),2) + math.pow((y - y_tem),2))
                if distance_min > distance_tmp and len(tem_drawer['text']):
                    distance_min = distance_tmp
                    index = i
            drawer = data[index]
            drawer_text = drawer['text']
...
def get_buyer_boundary(data):
    ...
    indexes = [i for i, d in enumerate(data) if i < end and re.search(r'(开票日期)|(校.?码)|(2?年?月)', d['text'])]
...
def is_wrapped_title2(data, line, content, next, boundary) :
    ...
    if dx < LEFT_MARGIN :
        if len(text) < 6 and len(line) : # len(tittle)==6 and
            ...
            if not re.search(r'\*[\u4e00-\u9fa5]', text) : # is number
