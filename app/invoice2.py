#!/usr/bin/env python
# -*- coding: utf-8 -*-
# author: pfq time: 2020/8/28 0028
import os
import cv2
import sys
import time
...
def recognize(im = None, path = None) :
    ...
    try :
        ...
        if im :
            ...
            dir = os.path.join(parent, 'data/images/invoice/')
...
def test(args, file = None) :
    ...
    for idx, (org_img, img) in enumerate(test_loader) :
        try :
            ...
            parent = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))
            dir_name = os.path.join(parent, 'data/images/tmp/')
            debug(idx, data_loader.img_paths, [[text_box]], dir_name)
            ...
            if args.evaluate :
                ...
                candidate_file = image_name + '-candidate.txt'
                parent = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))
                file = os.path.join(parent, 'data/gt/', image_file)
                errfile = os.path.join(parent, 'data/error/', error_file)
                canfile = os.path.join(parent, 'data/candidate/', candidate_file)
                if os.path.exists(file) :
                    # with open(canfile, 'w', encoding='utf-8') as f:
                    #     for data in result:
                    #         f.write(str(data) + '\n')
                    # f.close()
                ...
                if not iselectric :
                    ...
                    parent = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))
                    file = os.path.join(parent, 'data/error/images/', bg_file)
                    cv2.imwrite(file, bg_img)
                    fg_file = image_name + '_foreground.jpg'
                    file = os.path.join(parent, 'data/error/images/', fg_file)
...
def get_basic_checkcode(bg_data, fg_data) :
    ...
    if len(candidates) :
        ...
        if len(checkcodes) :
            ...
            candidate = checkcodes[0]['candidate']
    return checkcode,candidate
...
def get_basic_code(bg_data, fg_data) :
    ...
    if len(candidates) :
        ...
        if len(codes) :
            # code = codes[0]['text']
            code_candidate = codes[0]['candidate']
...
def get_basic_sn(bg_data, fg_data) :
    ...
    if len(candidates) :
        ...
        if len(codes) :
            # sn = codes[0]['text']
            sn_candidate = codes[0]['candidate']
...
def get_basic_person(bg_data, fg_data) :
    ...
    reviewer = ''
    drawer = ''
    payee_candidate, reviewer_candidate, drawer_candidate = ([], [], [])
    bg_rear = bg_data[-6 :]
    fg_rear = fg_data[-6 :]
    ...
    if len(candidates) :
        ...
        if len(payees) :
            payee = payees[0]['text']
            payee_candidate = payees[0]['candidate']
            if ':' in payee :
                len_title = len(payee.split(':')[0])
                payee = payee.split(':')[1]
                payee_candidate = payee_candidate[len_title+1:len_title+len(payee)]
    ...
    if len(candidates) :
        ...
        if len(reviewers) :
            ...
            reviewer_candidate = reviewers[0]['candidate']
            if ':' in reviewer :
                len_title = len(reviewer.split(':')[0])
                reviewer = reviewer.split(':')[1]
                reviewer_candidate = reviewer_candidate[len_title+1:len_title+len(payee)]
    ...
    if len(candidates) :
        ...
        if len(drawers) :
            ...
            drawer_candidate = drawers[0]['candidate']
            if ':' in drawer :
                len_title = len(drawer.split(':')[0])
                drawer = drawer.split(':')[1]
                drawer_candidate = drawer_candidate[len_title+1:len_title+len(payee)]
    return payee, payee_candidate, reviewer, reviewer_candidate, drawer, drawer_candidate


def getBasics(bg_data, fg_data) :
    checkcode, checkcode_candidate = get_basic_checkcode(bg_data, fg_data)
    type, title = get_basic_type(bg_data)
    code,code_candidate = get_basic_code(bg_data, fg_data)
    sn,sn_candidate = get_basic_sn(bg_data, fg_data)
    date,date_candidate = get_basic_date(bg_data, fg_data)

    payee, payee_candidate, reviewer, reviewer_candidate, drawer, drawer_candidate = get_basic_person(bg_data, fg_data)

    res = [[{'name' : r'发票类型', 'value' : type, 'candidate': []},
            {'name' : r'发票名称', 'value' : title, 'candidate': []},
            {'name' : r'发票代码', 'value' : code, 'candidate': code_candidate},
            {'name' : r'发票号码', 'value' : sn, 'candidate': sn_candidate},
            {'name' : r'开票日期', 'value' : date, 'candidate': date_candidate},
            {'name' : r'校验码', 'value' : checkcode, 'candidate': checkcode_candidate},
            {'name' : r'收款人', 'value' : payee, 'candidate': payee_candidate},
            {'name' : r'复核', 'value' : reviewer, 'candidate': reviewer_candidate},
            {'name' : r'开票人', 'value' : drawer, 'candidate': drawer_candidate}]]
    return res
...
def getBuyer(bg_data, fg_data) :
    ...
    name_candidate, taxnum_candidate, address_candidate, account_candidate = ([], [], [], [])
    ...
    if len(candidates) :
        ...
        if length == 4 :
            ...
            name_candidate, taxnum_candidate, address_candidate, account_candidate = [c['candidate'] for c in candidate]
        elif length == 2 :
            candidate = sorted(candidate, key = lambda x : x['cy'])
            name, taxnum = [c['text'] for c in candidate]
            name_candidate, taxnum_candidate = [c['candidate'] for c in candidate]
    res = [[{'name' : r'名称', 'value' : name, 'candidate': name_candidate},
            {'name' : r'纳税人识别号', 'value' : taxnum, 'candidate': taxnum_candidate},
            {'name' : r'地址、电话', 'value' : address, 'candidate': address_candidate},
            {'name' : r'开户行及账号', 'value' : account, 'candidate': account_candidate}]]
    return res
...
def getContent(bg_data, fg_data) :
    ...
    for r in res :
        ...
        if re.findall(r'[*]', title) :
            ...
            ret.append([{'name' : r'名称', 'value' : title,'candidate':[]},
                        {'name' : r'规格型号', 'value' : specs,'candidate':[]},
                        {'name' : r'单位', 'value' : unit,'candidate':[]},
                        {'name' : r'数量', 'value' : amount,'candidate':[]},
                        {'name' : r'单价', 'value' : uprice,'candidate':[]},
                        {'name' : r'金额', 'value' : price,'candidate':[]},
                        {'name' : r'税率', 'value' : ratio,'candidate':[]},
                        {'name' : r'税额', 'value' : tax,'candidate':[]}])
        else :
            continue
...
def merge_candidates(candidates) :
    ...
    while i < len(candidates) :
        if len(line) > 0:
            x = min(line,key = lambda x: x['cy'])
            y = candidates[i]
            if calc_axis_iou(x, y, 1) > 0.4 :
                line.append(y)
            if calc_axis_iou(x, y, 1) <= 0.4 or i == len(candidates)-1:
                ...
                for j in range(len(line)):
                    if temp:
                        # print(temp)
                        # print(line[j])
                        if abs(temp['cx'] + temp['w'] / 2 - line[j]['cx'] + line[j]['w'] / 2) < 20:
                            temp['text'] += y['text']
                            temp['candidate'].extend(y['candidate'])
                        if abs(temp['cx'] + temp['w'] / 2 - line[j]['cx'] + line[j]['w'] / 2) >= 20:
                            merged.append(temp)
                            temp = line[j]
                    else:
...
def getSeller(bg_data, fg_data) :
    ...
    # print([d['text'] for d in candidates])
    if len(candidates) :
...
def get_summation_boundary(bg_data, fg_data) :
    ...
    summation = [i for i, d in enumerate(fg_data) if re.search(r'(￥|Y|羊|F|单|爱)\d+?\.?\d*[^\d\.]?\d*$', d['text'])]
...
def getSummation(bg_data, fg_data) :
    ...
    prices_data = [ d for d in summation if
              re.search(r'(￥|Y|羊|F|爱|单)?\d+?\.?\d*', d['text']) and len(d['text']) > 2]
    for i in range(len(prices_data)):
        prices.append(re.split('￥|Y|羊|F|爱|单',prices_data[i]['text'])[1])
        candidate = prices_data[i]['candidate'][-1*len(prices[i]):]
        for j in range(len(prices[i])):
            if (prices[i][j] < '0' or prices[i][j] > '9') and prices[i][j] != '.':
                for k in range(len(candidate[j])):
                    if(candidate[j][k] >= '0' and candidate[j][k] <= '9') or candidate[j][k] == '.':
                        prices[i] = prices[i][:j] + candidate[j][k] + prices[i][j+1:]
                        break
    prices = [re.sub(r'[^\d\.]', '', d) for d in prices]
    ...
    res = [[{'name' : r'金额合计', 'value' : price, 'candidate' : []},
            {'name' : r'税额合计', 'value' : tax, 'candidate' : []},
            {'name' : r'价税合计(大写)', 'value' : capital, 'candidate' : []},
            {'name' : r'价税合计(小写)', 'value' : total, 'candidate' : []}]]
    return res
