...
def getBasics(data):
    s, e, left, right = get_basic_boundary(data) # start end
    basic = data[s:e+1]
    ...
    payee, reviewer, drawer = get_basic_person(data, [left, right]) # 这里用的是 左和you的信息
...
def get_buyer_boundary(data):
    ...
    print(data[21]),print(data[22]),print(data[23]),print(data[24]),print(data[25]),print(data[26])
    end = min(indexes)
...
def formatResult(data): # jin
    basic = getBasics(data)  # 发票基本信息
    buyer = getBuyer(data) # 卖家信息
    content,calcsum = getContent(data) #
    seller = getSeller(data) # 卖家信息
