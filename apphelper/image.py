...
def sort_box(box):
    ...
    对box排序,及页面进行排版
        ...
        box[index, 7] = y4
    """
    # box = [b for b in box if abs(b[0]-b[2]) >= 0.75*abs(b[1] - b[7])]
    # box = sorted(box,key=lambda x:sum([x[1],x[3],x[5],x[7]]))
    # box = [b for b in box if b[0] > -45]
    # box = sorted(box, key=lambda x:x[4])

    # n = len(box)
    # for i in range(0, n):
    #     y1 = (box[i][1]+box[i][3])/2
    #     y2 = (box[i][5]+box[i][7])/2
    #     for j in range(i + 1, n):
    #         y3 = (box[j][1]+box[j][3])/2
    #         y4 = (box[j][5]+box[j][7])/2
    #         iou = calc_iou([y1,y2],[y3,y4])
    #         if iou < 0.1:
    #             if j - i > 1:
    #                 temp = sorted(box[i:j], key=lambda x:x[0])
    #                 box[i:j]=temp
    #             i = j
    return box
    ...
    #         b2 = box[i]
    #         if abs(b1[1] - b2[1]) < 10 or abs(b1[3] - b2[3]) < 10 or abs(b1[5] - b2[5]) < 10 or abs(b1[7] - b2[7]) < 10:
    #             a1.append(b2)
    #         else:
    #             a2.append(b2)
    #     if len(a1) > 1:
    #         a1 = sorted(a1, key=lambda x:x[0])
    #     res.extend(a1)
    #     box = a2
    #
    # return list(res)
